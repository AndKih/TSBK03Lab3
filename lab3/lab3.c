...
void updateWorld()
{
	...
	for (i = 0; i < kNumBalls; i++)
		...
		if(Norm(VectorSub(ball[i].X, ball[j].X)) < 2*r && DotProduct(speedDiff, normal) < 0)
		{
		      ...
		      vec3 pointSpeedi = VectorAdd(speedi, CrossProduct(ball[i].omega, ri));
		      vec3 speedj = ScalarMult(ball[j].P, 1/ball[j].mass);
		      vec3 pointSpeedj = VectorAdd(speedj, CrossProduct(ball[j].omega, rj));
		      vec3 speedRel = VectorSub(pointSpeedi, pointSpeedj);
		      float speedRelProj = DotProduct(speedRel, normal);
		      ...
		      printVec3(ScalarMult(otherNormal, deltaVj));*/
		      /*if(i > 4)
		      {
			    printf("PRE\n");
			    printVec3(ball[i].P);
...
void init()
{
	...
	float start = 1.5f, baseOffset = 0.25f, offset = 0.0f;
	float xcoord = -0.2f;
	for (i = 0; i < kNumBalls; i++)
	{
		ball[i].mass = 1.0;
